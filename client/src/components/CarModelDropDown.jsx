...
<Listbox value={selected} onChange={setSelected}>
      <Label className="block text-lg font-semibold txt-shadow text-slate-200">Car Type</Label>
      <div className="relative mt-2">
        ...
        >
          {model.map((model) => (
            <ListboxOption
              key={model.id}
              value={model} onClick={()=>setSelectedModel(model.name)}
            ...
            >
              <div className="flex items-center">
                <span className=" block truncate font-normal group-data-selected:font-semibold sm:text-xl">{model.name}</span>
              </div>
