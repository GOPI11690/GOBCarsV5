...
const addCar=asyncHandler(async(req, res) => {
    const {name,brand,type,fuel,capacity,gear,status,rateperday}=req.body;
    //car data validation
    if(!name||!brand||!type||!fuel||!capacity||!rateperday){
        res.status(400);
        throw new Error("Please enter all the details");
    }
    //adding car data
    const car=await CarModel.create({
        name,brand,type,fuel,capacity,gear,status,rateperday,
        userid:req.user.id,
    });
